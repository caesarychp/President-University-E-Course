			...
			<div class="container-fluid">
				<table class="table table-bordered table-stripped">
					...
					<tbody>
						<?php
						$i = 1;
						$qry = $conn->query("SELECT r.*, c.category, c.balance, v.vendor_name
						FROM `running_balance` r
						INNER JOIN `categories_expenses` c ON r.category_id = c.id
						INNER JOIN `vendor` v ON r.vendor_id = v.vendor_id
						WHERE c.status = 1 AND r.balance_type = 2
						...
						?>
							<tr>
								...
								<td>
									<p class="m-0 text-right"><?php echo number_format($row['amount']) ?></p>
								</td>
								<td>
