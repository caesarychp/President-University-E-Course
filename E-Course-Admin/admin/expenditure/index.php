...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title">List of Expenditure</h3>
			...
			<div class="container-fluid">
				<table class="table table-hover table-striped">
					...
					<tbody>
						<?php
						$i = 1;
						$qry = $conn->query("SELECT po.*, v.vendor_name FROM `purchase_order` po INNER JOIN `vendor` v ON po.vendor_id = v.vendor_id ORDER BY unix_timestamp(po.date_updated)");
						while ($row = $qry->fetch_assoc()) :
							$row['item_count'] = $conn->query("SELECT * FROM purchase_order where po_id = '{$row['id']}'")->num_rows;
							$row['total_amount'] = $conn->query("SELECT sum(quantity * price) as total FROM purchase_order where po_id = '{$row['id']}'")->fetch_array()['total'];
						?>
							<tr>
								<td class="text-center"><?php echo $i++; ?></td>
								<td class=""><?php echo date("M d,Y H:i", strtotime($row['date_created'])); ?></td>
								<td class=""><?php echo "PO" . $row['po_id'] ?></td>
								<td class=""><?php echo $row['vendor_name'] ?></td>
