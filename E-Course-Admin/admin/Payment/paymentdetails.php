...
if (isset($_GET['ap_id']) && $_GET['ap_id'] > 0) {
    $qry = $conn->query("SELECT * from `accounts_payable` where ap_id = '{$_GET['ap_id']}' ");
    if ($qry->num_rows > 0) {
...
<div class="div">
    ...
    <!-- Button to load Banks page -->
    <div class="div-3">
        <button class="div-4 banks">BANKS</button>
    </div>

    <!-- Button to load E-Wallet page -->
    <div class="div-3">
        <button class="div-4" onclick="loadEWallet()">E-WALLET</button>
    </div>
...
<script>
    $(document).ready(function() {
        $('.banks').click(function() {
            // Append po_id to the URL
            uni_modal_view("<i class='fa fa-plus'></i> Banks", "payment/banks.php?id=<?php echo isset($ap_id) ? $ap_id : '' ?>");
        });
