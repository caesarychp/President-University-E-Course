        ...
        <div id="printable">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    $i = 1;
                    $total = 0;
                        $qry = $conn->query("SELECT r.*,c.category,c.balance from `running_balance` r inner join `categories_expenses` c on r.category_id = c.id where c.status=1 and r.balance_type = 2 and date(r.date_created) between '{$date_start}' and '{$date_end}' order by unix_timestamp(r.date_created) asc");
...
<script>
    $(function(){
        $('#filter-form').submit(function(e){
            e.preventDefault()
            location.href = "./?page=reports/expenditure&date_start="+$('[name="date_start"]').val()+"&date_end="+$('[name="date_end"]').val()
        })
